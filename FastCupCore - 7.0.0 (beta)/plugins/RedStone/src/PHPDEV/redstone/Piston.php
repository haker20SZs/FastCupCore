...
use pocketmine\block\Piston as Piston2;
...
class Piston extends Activateble{
	...
	public function onActivate($x, $y, $z, &$checked){
		...
		if($block instanceof Piston2 && $this->canExtend($block, 0)){
			$this->extend($block);
	...
	public function onDeactivate($x, $y, $z, &$checked){
		$block = $this->getServer()->getDefaultLevel()->getBlock(new Vector3($x,$y,$z));
		if($block instanceof Piston2 && empty($this->notactivated[$block->asVector3()->__toString()])){
			$this->retract($block);
		}
	...
	public function retract($block) {
		...
		$extendBlock = $block->getSide($block->getExtendSide());
		if ($extendBlock->getId() == 34) $block->getLevel()->setBlock($extendBlock, Block::get(Block::AIR), true, true);
		$tile->spawnToAll();
	}
